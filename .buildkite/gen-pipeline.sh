...
commit_files=$(python $(dirname "$0")/get_pull_request_files.py)
non_code_files=$(echo "$commit_files" | grep -v -e "^.buildkite" -e "^.buildkite/get_pull_request_files.py$" -e "^.github/" -e "^docs/" -e "^*.md" -e "^*.rst" || true)
echo "$non_code_files" >&2
tests=$(if [[ -z "$commit_files" || "${BUILDKITE_BRANCH:-}" == "${BUILDKITE_PIPELINE_DEFAULT_BRANCH:-}" ]] || [[ -n "$non_code_files" ]]; then
